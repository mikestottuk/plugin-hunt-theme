	...
	static function optionsframework_tabs() {
		...
		foreach ( $options as $value ) {
			...
			if ( $value['type'] == "heading" ) {
				...
				$menu .= '<li><a id="options-group-'.  $counter . '-tab" class="nav-tab-epic ' . $class. ' ' .$act .'" title="' . esc_attr( $value['name'] ) . '" data-tab="' . esc_attr( 'options-group-'.  $counter ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '">' . esc_html( $value['name'] ) . '</a></li>';
			}
		}
	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
				...
				if ( isset( $value['name'] ) ) {
					$output .= '<div class="format-setting-label"><span class="heading epic-label">' . esc_html( $value['name'] ) . '</span></div>' . "\n";
				}
...
*/
				...
				$output .= '<div class="onoffswitch"><input id="' . esc_attr( $value['id'] ) . '" class="onoffswitch-checkbox checkbox of-input" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" '. checked( $val, 1, false) .' />';
        ...
        <span class="onoffswitch-switch"></span></label></div>';
			...
			case "heading":
				...
				$output .= '<div id="options-group-' . $counter . '" class="group ' . $class . ' ' . $act . '">';
